...
class FyersBrokerV2(DataBroker):
    ...
    def get_historical_data(
        self,
        symbol: str,
        start_date_or_res: str | date | None = None,
        end_date_or_from: str | date | None = None,
        ...
        """
        # Resolve inputs to handle keyword arguments passed from the new factory interface
        s_date = start_date if start_date is not None else start_date_or_res
        e_date = end_date if end_date is not None else end_date_or_from
        t_frame = timeframe if timeframe is not None else timeframe_or_to
