...
class MarketPatternAnalyzer:
    ...
    def build_daily_features(self, intraday: pd.DataFrame) -> pd.DataFrame:
        ...
        df["week_key"] = pd.to_datetime(df["trade_date"], format="mixed").dt.to_period("W-FRI").astype(str)
        ...
        df["month_key"] = pd.to_datetime(df["trade_date"], format="mixed").dt.to_period("M").astype(str)
    ...
    def build_multi_timeframe_levels(self, intraday: pd.DataFrame, daily_features: pd.DataFrame) -> pd.DataFrame:
        trade_date = pd.to_datetime(daily_features["trade_date"], format="mixed")
