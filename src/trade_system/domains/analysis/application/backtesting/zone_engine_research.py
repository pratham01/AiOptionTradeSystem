...
from trade_system.domains.analysis.application.backtesting.ict_fvg_liquidity_research import FVGDetector, OrderBlockDetector
...
class ZoneEngineResearch:
    ...
    def _backtest(self, file_path: Path) -> pd.DataFrame:
        ...
        if not frame.empty:
            frame["holding_minutes"] = (
                pd.to_datetime(frame["exit_time"], format="mixed") - pd.to_datetime(frame["entry_time"], format="mixed")
    ...
    def _build_report(self, year: int, summary: pd.DataFrame, trades: pd.DataFrame) -> str:
        ...
        if not trades.empty:
            ...
            monthly_frame["month"] = pd.to_datetime(monthly_frame["entry_time"], format="mixed").dt.to_period("M").astype(str)
            monthly = (
