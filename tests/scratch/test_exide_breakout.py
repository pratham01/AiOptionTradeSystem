...
# Fetch Exide data for today and past 10 days
query = text("""
    SELECT symbol, timestamp, open, high, low, close, volume 
    FROM ohlcv_15m 
    WHERE symbol='NSE:EXIDEIND-EQ' 
...
if not today_candles.empty:
    ...
    for i in range(len(today_candles)):
        ...
        is_vol_surge = vol > (1.5 * prev_vol_sma) # Using 1.5x threshold now!
        ...
        print(f"  Vol SMA 20: {prev_vol_sma:.1f} | Ratio: {vol_ratio:.2f}x (Surge at 1.5x: {is_vol_surge})")
