...
def test_tatamotors():
    print("Sleeping for 5 seconds to reset any rate limits...")
    time.sleep(5)
    ...
    print("Testing string format for NSE:TATAMOTORS-EQ...")
    ...
    print("\nTesting epoch format for NSE:TATAMOTORS-EQ...")
    res_epoch = broker.fyers.history(data=payload_epoch)
    print(f"Epoch format result: success={res_epoch.get('s')}, candles_count={len(res_epoch.get('candles', []))}")
    if res_epoch.get('s') != 'ok':
        print(f"Error details: {res_epoch}")
